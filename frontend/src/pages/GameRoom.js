...
const GameRoom = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white pb-28 md:pb-4">
      {/* Header */}
          ...
          <div className="flex items-center justify-between">
            ...
            <Button
              ...
              onClick={() => {
                ...
                toast.info(showChat ? 'Chat closed' : 'Chat opened');
              }}
              className={`text-white hover:bg-white/20 relative ${showChat ? 'bg-white/20' : ''}`}
      ...
      <div className="container mx-auto px-4 py-4 max-w-5xl">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="space-y-4">
            ...
            {/* Chat - Desktop (hidden on mobile, shown in sidebar) */}
            {showChat && (
              <Card className="bg-slate-900/80 border-slate-700 hidden lg:block">
                <CardHeader className="pb-2">
      ...
      {/* Chat Modal - Mobile Only (appears as overlay) */}
      {showChat && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          {/* Chat Panel */}
          <div className="absolute bottom-0 left-0 right-0 bg-slate-900 border-t border-slate-700 rounded-t-2xl max-h-[70vh] flex flex-col animate-slideUp">
            {/* Header */}
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2 min-h-[200px] max-h-[400px]">
              {chatMessages.length === 0 ? (
                ...
                chatMessages.map((msg, idx) => (
                  <div 
                    key={idx} 
                    className={`p-2 rounded-lg ${msg.isMe ? 'bg-yellow-500/20 ml-8' : 'bg-slate-800 mr-8'}`}
                  >
                    <span className={`text-xs font-medium ${msg.isMe ? 'text-yellow-400' : 'text-blue-400'}`}>
                      {msg.isMe ? 'You' : msg.player_name}
                    </span>
            ...
            <div className="p-4 border-t border-slate-700 bg-slate-900">
              <div className="flex gap-2">
                ...
                <Button 
                  onClick={sendChat} 
                  className={`bg-gradient-to-r ${gameInfo.color}`}
                  data-testid="send-chat-btn-mobile"
                >
                  <Send className="h-4 w-4" />
                </Button>
              </div>
